...
- **프로젝트 이름을 `mason-recap`에서 `mason-report`로 바꿨다.** 저장소 내 모든 파일·코드
  레퍼런스(플러그인 디렉터리, 커맨드 접두사 `/mason-report:*`, 로컬 로그 디렉터리
  `.mason-report/`, npm 패키지명, 문서·예시)를 일괄 교체했다. **호환성이 깨지는 변경이다**
  — 기존 설치자는 새 마켓플레이스/플러그인 이름으로 다시 설치해야 한다.
- **마켓플레이스 이름과 플러그인 이름을 분리했다.** 마켓플레이스는 `mason`, 플러그인은
  `mason-report`로 설치 시 `mason-report@mason` 형태로 참조한다(과거에는 마켓플레이스와
  플러그인 이름이 우연히 같은 문자열이었다).
- 기존에 수집된 로컬 관찰 로그(`.mason-recap/`)를 잃지 않도록, 이 저장소를 포함해 로컬에
  존재하던 프로젝트들의 로그 디렉터리를 `.mason-report/`로 옮겼다. 이미 열려 있는
  세션의 캐시된 플러그인이 새로고침 전까지 계속 `.mason-recap/`에 쓸 수 있어, 과도기
  동안 두 디렉터리 모두 `.gitignore`에 남겨뒀다.

## [0.1.9] - 2026-09-18

### Added

- **`/mason-recap:readme` 커맨드를 추가했다.** 로그를 조회하지 않는 정적 안내 커맨드로,
